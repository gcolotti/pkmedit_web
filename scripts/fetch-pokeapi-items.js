#!/usr/bin/env node
// Fetches PokeAPI item list, downloads missing sprites to public/assets/items/,
// and prints new ID->localPath entries for wikiDexAssets.ts
...
// IDs already covered by WikiDex downloads
...
async function main() {
  console.error('Fetching PokeAPI item list...')
  const list = await fetchJson(
    'https://pokeapi.co/api/v2/item?limit=2000&offset=0',
  )
  console.error(`Got ${list.results.length} items`)

  // Build id→name map from URL
  const idToName = {}
  ...
  for (const id of ids) {
    ...
    if (ok) {
      console.error('ok')
      newEntries.push({ id, path: `/assets/items/${id}.png` })
    } else {
      console.error('MISS')
    }
  }

  // Print TypeScript entries for wikiDexAssets.ts
  console.log('\n// --- PokeAPI supplements (auto-generated) ---')
  for (const { id, path: p } of newEntries) {
    console.log(`  ${id}: '${p}',`)
  }
